/**
 * Resolves the backend test-harness environment.
 *
 * Backend unit/property tests run without external services; integration tests
 * (added in later tasks) run against a dedicated test Postgres + Redis. The
 * connection strings come from `TEST_DATABASE_URL` / `TEST_REDIS_URL` (see the
 * repo `.env.example`), falling back to the documented local defaults so the
 * harness is runnable out of the box.
 */
